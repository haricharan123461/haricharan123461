## Hi there 👋

[![](https://komarev.com/ghpvc/?username=haricharan123461&label=Actual%20Profile%20Views&color=0e75b6&style=flat)](https://github.com/haricharan123461)

<!-- The Komarev badge tracks real views, and the second badge mimics a view count starting from 1k -->

🎓 I'm currently pursuing a **Bachelor of Engineering in Information Technology (2022–2026)** at Chaitanya Bharathi Institute of Technology, Hyderabad (CGPA: 8.8).  
💡 I'm passionate about **full stack development**, **generative AI**, and building real-world solutions using **cloud services** and **deep learning** technologies.

---

### 🧠 Technical Skills

- **Languages:** Java, Python, Kotlin, JavaScript, HTML, CSS  
- **Frontend:** React.js, Sass, Bootstrap  
- **Backend:** Node.js, Express.js  
- **Databases:** MongoDB, MySQL  
- **Cloud & DevOps:** Google Cloud Platform (GCP), Git, GitHub  
- **Tools:** VS Code, Android Studio, Postman  
- **Frameworks & Libraries:** LangChain, Gradio, Apache Spark, Python NLP (nltk, TextBlob, transformers)  

---

### 🧾 Certifications

- **Generative AI by Google Cloud Skills Boost (Aug–Sep 2024)**  
  > Built real-world apps using Gemini APIs, Vertex AI, and multimodal RAG.

---

### 📚 Relevant Courses

- Data Structures & Algorithms, OS, OOP, DBMS  
- Cloud Computing, Deep Learning, Software Engineering  
- Big Data Analytics, Generative AI, Machine Learning  

---

### 📫 Contact Me

- Email: jatothharicharan@gmail.com  
- GitHub: [@haricharan123461](https://github.com/haricharan123461)  
- Location: Hyderabad, Telangana, India  
- 📱 +91-7981296071  

---

✨ *“Building AI-driven full-stack systems to solve real-world problems.”*

